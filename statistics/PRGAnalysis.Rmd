...
A procedural algorithm for generating syllables from this pattern should be able to work through the syllable from front to back, so we also need to obtain the proportion of fully closed syllables given a closed front. Since these variables are dependent we use Baye's Theorem:

$$P(A | B) = P(A & B)/P(B)$$

$$P(both | front) = P(both & front)/P(front)$$

$$P(both | front) = P(front)* P(front|both)/P(front)$$
...
The expected proportion of all syllables with a closed front is between about 77.9%, and the proportion of all closed syllables is about 53.1%. The probability of a syllable having a closed front given being closed on both sides is 1, so plugging in yields

$$P(both | front) = P(front)* 1/P(front)$$
